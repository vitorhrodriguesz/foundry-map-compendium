(() => {
  ...
  Hooks.once("init", () => {
    ...
    game.settings.register(MODULE_ID, "catalogSchema", {
  ...
  Hooks.once("ready", async () => {
    ...
    const initialized = game.settings.get(MODULE_ID, "catalogInitialized");
    const schema = game.settings.get(MODULE_ID, "catalogSchema");
    if (!initialized || schema !== CATALOG_SCHEMA) {
      try {
        await syncCatalog({ quiet: false });
      } catch (err) {
        console.error(`${MODULE_TITLE} | Falha na sincronização inicial`, err);
        ui.notifications?.error(`${MODULE_TITLE}: não foi possível montar o catálogo. Use “Sincronizar Mapas” na aba Cenas.`);
      }
    }
  });
  ...
  Hooks.on("renderSceneDirectory", (app, html) => {
    ...
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        ui.notifications?.error(`${MODULE_TITLE}: falha ao sincronizar. Veja o console (F12).`);
      } finally {
  ...
  Hooks.on("createScene", (scene, options, userId) => {
    ...
    setTimeout(() => hydrateScene(scene), 75);
  });
  ...
  async function syncCatalog({ quiet = false } = {}) {
    ...
    await game.settings.set(MODULE_ID, "catalogSchema", CATALOG_SCHEMA);
    await game.settings.set(MODULE_ID, "lastSync", new Date().toISOString());
    if (!quiet) ui.notifications?.info(`${MODULE_TITLE}: ${entries.length} mapas catalogados; ${added} novos adicionados.`);
  ...
  async function ensurePack(category) {
    ...
    if (!CompendiumClass) throw new Error("API de Compendium não encontrada no Foundry v13.");
    return CompendiumClass.createCompendium({
      name,
      ...
      flags: {
        [MODULE_ID]: {
          catalog: {
            ...
            tags: buildTags(entry.path, category),
  ...
  async function hydrateScene(scene) {
    ...
    try {
      ...
      const width = Math.round(Number(uvtt.resolution.map_size.x) * ppg);
      const height = Math.round(Number(uvtt.resolution.map_size.y) * ppg);
      ...
      if (!FP) throw new Error("API FilePicker não encontrada no Foundry v13.");
      await FP.upload("data", folder, uploadFile, {}, { notify: false });
      ...
      const walls = buildWalls(uvtt, ppg);
      const doors = buildDoors(uvtt, ppg);
      const lights = buildLights(uvtt, ppg);
      ...
      if (ppg < originalPpg) {
        ui.notifications?.warn(`${MODULE_TITLE}: “${scene.name}” foi reduzido de ${originalPpg}px para ${ppg}px por quadrado para respeitar limites do navegador.`);
      }
  ...
  function buildLights(uvtt, ppg) {
    const origin = uvtt.resolution.map_origin || { x: 0, y: 0 };
    const gridDistance = Number(game.system?.grid?.distance) || Number(globalThis.canvas?.scene?.grid?.distance) || 5;
    const out = [];
